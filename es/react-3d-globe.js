...
var Globe = function () {
  ...
  Globe.prototype.addMarkers = function addMarkers(markers) {
    ...
    var pointScale = scaleLinear().domain([minVal, maxVal]).range([10, 20]);
    markers.forEach(function (marker) {
      ...
      switch (marker.type) {
        ...
        case 'point':
          ...
          mesh = new Mesh(new SphereGeometry(size, size, size / 2), material);
          break;
        default:
  ...
  Globe.prototype.focus = function focus(to) {
    ...
    new Tween(from).to(to, 600).easing(Easing.Linear.None).on('update', function () {
  ...
  Globe.prototype.unfocus = function unfocus() {
    ...
    new Tween(cameraPosition).to(this.preFocus, 600).easing(Easing.Linear.None).on('update', function () {
      self.camera.position.set(this.object.x, this.object.y, this.object.z);
      self.camera.lookAt(new Vector3(0, 0, 0));
    }).on('complete', function () {
      self.camera.lookAt(new Vector3(0, 0, 0));
  ...
  Globe.prototype._createCamera = function _createCamera() {
    ...
    var camera = new PerspectiveCamera(viewAngle, this.aspect, near, far);
